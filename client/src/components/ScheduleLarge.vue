      ...
      getResultOrStyle(event, heroTeam = null, type = 'result') {
        ...
        switch (type) {
          ...
          default:
            if (!heroTeam)
              return `${
                heroTeamScore > villainTeamScore ? 'H' : 'V'
              } ${villainTeamScore}-${heroTeamScore}`
            if (isGameFinal) {
    ...
    created() {
      ...
      this.scheduleData = this.events.map((ev) => {
        ...
        return {
          ...
          visiting_team: {
            uid: ev.visiting_api_uid,
            name: ev.visiting_full_name,
            logo: ev.visiting_local_filename,
          },
          home_team: {
            uid: ev.home_api_uid,
            name: ev.home_full_name,
            logo: ev.home_local_filename,
          },
          spread: ev.odds_spread,
          over_under: ev.odds_over_under,
          result: this.getResultOrStyle(ev, this.team),
          TV_listings:
            tvListings && tvListings.us
              ? tvListings.us.map((n) => n.short_name).join(', ')
              : 'N/A',
        }
      })
    },
